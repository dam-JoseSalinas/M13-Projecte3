...
import CalendarioDoctor from './screens/profile/CalendarioDoctor';
...
function DrawerNavigator({ darkMode }) {
    ...
    useEffect(() => {
        // Actualizamos user solo cuando haya datos válidos
        if (userData.email.endsWith('@joyclinc.es')) {
            setUser(userData);
        } else {
            setUser(userDoctor);
        }
    }, [userData, userDoctor]); // Dependencias de useEffect para actualizar user
    return (
        <Drawer.Navigator
            drawerContent={(props) => (
