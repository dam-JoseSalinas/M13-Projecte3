...
import styles from '../../assets/styles/contactsDoctorStyles';
import config from '../settings/Config';

export default function ContactsDoctor() {
    const [contacts, setContacts] = useState([]);
    const [docs, setDocs] = useState([]);
    const [text, setText] = useState('');
    const navigation = useNavigation();
    const { userDoctor, usersData, fetchData, fetchDoctor } = useContext(ProfileContext);
    ...
    const filteredContacts = useMemo(() => {
        ...
        return allContacts.filter(contact => {
            ...
            const isCurrentUser = contact.id === userDoctor.id;
        ...
        <SafeAreaView style={[{ backgroundColor: theme.theme }, { flex: 1 }]}>
            ...
            <View style={[styles.fondoNombre, { backgroundColor: theme.background }, { borderColor: theme.lineColor }]}>
                <View style={[styles.foto, { backgroundColor: theme.background }, { borderColor: theme.lineColor }]}>
                    <Image
                        source={userDoctor.photo ? { uri: config.loginAPI + userDoctor.photo } : profileImage}
                ...
                <View>
                    <Text style={[styles.nombre, { color: theme.color }]}>
                        {userDoctor.name} {userDoctor.surname}
                    </Text>
                    <Text style={[styles.nombre, { color: theme.color }]}>
                        {userDoctor.email}
                    </Text>
