...
def contar_citas(request):
    ...
    try:
        doctor_count = Appointments.objects.filter(of=doctor_instance).count()
        return Response({doctor_count})
    except Exception as e:
        print(e)
        return Response({"success": False, "message": "Internal Server Error"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def contar_citas_paciente(request):
    try:
        pacient_instance = Register.objects.get(email=request.user.email)
    except Register.DoesNotExist:
        return Response({"message": "Profile not found."}, status=status.HTTP_404_NOT_FOUND)
    
    try:
        paciente_count = Appointments.objects.filter(forWhom=pacient_instance).count()
        return Response({paciente_count})
    except Exception as e:
        print(e)
        return Response({"success": False, "message": "Internal server Error"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def appointments_for_patient(request):
    try:
        # Obtener el perfil del paciente basado en el usuario autenticado
        patient_instance = Register.objects.get(email=request.user.email)
    except Register.DoesNotExist:
        return Response({"message": "Patient not found."}, status=status.HTTP_404_NOT_FOUND)

    try:
        # Filtrar citas basadas en el paciente
        patient_appointments = Appointments.objects.filter(forWhom=patient_instance)
